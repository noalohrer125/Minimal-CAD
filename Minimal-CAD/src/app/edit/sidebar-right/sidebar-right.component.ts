...
export class SidebarRightComponent implements OnInit {
  ...
  private createCommandGroup(cmd?: FreeObjectCommand): FormGroup {
    const group: any = {
      type: new FormControl(
        { value: cmd?.type ?? 'lineTo', disabled: false },
        Validators.required,
      ),
  ...
  initForm(): void {
    ...
    this.selectedObject =
      this.drawService.loadObjects().find((obj) => obj.selected) || null;
    ...
    if (this.selectedObject) {
      ...
      if (
        this.selectedObject.type === 'Square' ||
        ...
        patch.size = {
          ...
          ...(this.selectedObject.type === 'Circle'
            ? { curveSegments: this.selectedObject.curveSegments ?? 100 }
            : {}),
        };
        patch.position = {
  ...
  private saveToLocalStorage() {
    ...
    if (this.selectedObjectType === 'Square') {
      ...
      const commands = (this.form.value.commands || [])
        .map((cmd: any) => {
    ...
    const existingIndex = modelData.findIndex(
      (model: any) => model.id === this.selectedObject.id && !model.ghost,
    );
    if (existingIndex !== -1) {
  ...
  async onClose() {
    try {
      ...
      await this.dialogService.alert(
        'Error',
        'Sidebar could not be closed. Please try again.',
      );
    }
  }
  ...
  async onDelete() {
    ...
    if (shouldDelete) {
      try {
        ...
        await this.dialogService.alert(
          'Error',
          'Object could not be deleted. Please try again.',
        );
      }
    }
  }
