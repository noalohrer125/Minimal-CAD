  ...
  implements OnInit, OnChanges, AfterViewInit, OnDestroy
{
  ...
  @Input() set cameraReset(val: {
    ...
    if (val) {
      ...
      this.sceneService.setCameraRotation(
        val.rotation.x,
        val.rotation.y,
        val.rotation.z,
      );
      this.animationService.setTargetScale(
        new THREE.Vector3(val.scale.x, val.scale.y, val.scale.z),
      );

      if (val.rootGroupPosition) {
        this.animationService.setTargetPosition(
          new THREE.Vector3(
            val.rootGroupPosition.x,
            val.rootGroupPosition.y,
            val.rootGroupPosition.z,
          ),
        );
      }
  ...
  init() {
    ...
    this.sceneService.setRootGroupPosition(
      view.rootGroup.position.x,
      view.rootGroup.position.y,
      view.rootGroup.position.z,
    );
    this.sceneService.setRootGroupRotation(
      view.rootGroup.rotation.x,
      view.rootGroup.rotation.y,
      view.rootGroup.rotation.z,
    );
  }
  ...
  loadModels() {
    ...
    modelData.forEach((el) => {
      if (el.type === 'Freeform') {
        this.modelRenderService.renderFreeFormObject(
          el,
          rootGroup,
          el.selected,
          el.ghost || false,
        ...
        this.modelRenderService.renderFormObject(
          el,
          rootGroup,
          el.selected,
          el.ghost || false,
  ...
  async ngAfterViewInit(): Promise<void> {
    try {
      ...
      this.dialogService.alert(
        'Error',
        'Failed to initialize the view. Please reload the page.',
      );
    }
  }

  private async loadProjectObjects(): Promise<void> {
    if (!this.projectId) {
      return;
    }
    ...
    try {
      ...
      console.error('Error loading project objects:', error);
      this.dialogService.alert(
        'Error',
        'Failed to load project objects. Please try again.',
      );
    } finally {
