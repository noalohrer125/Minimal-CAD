...
import {
  ...
  newtestemail,
  newtestpassword,
} from '../shared/testing/testuser-credentials';
    ...
    it('should not register with existing email', (done) => {
      ...
      service.register(testemail, 'anotheruser', 'newpassword').subscribe({
        next: () => {
            ...
            new Error(
              'Expected error for existing email, but registration succeeded',
            ),
          );
        },
        error: (err) => {
      ...
      service.login('invalidemail', 'invalidpassword').subscribe({
        next: () => {
          service.logout().subscribe();
            ...
            new Error('Expected error for invalid login, but login succeeded'),
          );
        },
        error: (err) => {
          expect(err).toBeDefined();
